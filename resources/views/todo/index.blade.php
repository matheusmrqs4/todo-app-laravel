<x-app-layout>
        ...
        <ul
            ...
            <li>
                <a href="{{ route('todos.create') }}"
                    class="block py-2 pl-3 pr-4 md:text-blue-400 md:p-0 hover:text-blue-200">Nova Tarefa</a>
            </li>
        </ul>
    </div>
                    ...
                    <tr class="bg-gray-800 dark:bg-gray-800 dark:border-gray-700 text-center">
                        ...
                        <td class="px-6 py-4 text-center">
                            <a href="{{ route('todos.show', ['todo' => $todo->id]) }}"
                                class="font-medium text-blue-400 dark:text-blue-500 hover:underline">Ver
                                Detalhes</a>
                        </td>
                    </tr>
                @endforeach
    ...
    <div class="mt-2 flex justify-center">
        {{ $todos->appends($request)->links('todo.layouts.components.simple-tailwind') }}
    </div>
    <div class="mt-2 flex justify-center text-gray-600">
        Exibindo {{ $todos->count() }} Produtos de {{ $todos->total() }} (de
        {{ $todos->firstItem() }} a {{ $todos->lastItem() }})
    </div>
    <x-footer></x-footer>
</x-app-layout>
